...
// .wrangler/tmp/bundle-UCjCsK/strip-cf-connecting-ip-header.js
...
// src/oauth.ts
var X_AUTH = "https://twitter.com/i/oauth2/authorize";
var X_TOKEN = "https://api.x.com/2/oauth2/token";
function base64urlRandom(bytes = 32) {
  const arr = new Uint8Array(bytes);
  crypto.getRandomValues(arr);
  return btoa(String.fromCharCode(...arr)).replaceAll("+", "-").replaceAll("/", "_").replaceAll("=", "");
}
__name(base64urlRandom, "base64urlRandom");
async function sha256Base64Url(plain) {
  const encoder2 = new TextEncoder();
  const data = encoder2.encode(plain);
...
var oauth = {
  async createAuthUrl(clientId, redirectUri) {
  ...
  async exchangeCodeForTokens(code, codeVerifier, clientId, clientSecret, redirectUri) {
  ...
  async refreshTokens(refreshToken, clientId, clientSecret) {
    const tokenParams = new URLSearchParams({
  ...
  async createJWT(userId, secret) {
    return new SignJWT({ sub: userId }).setProtectedHeader({ alg: "HS256" }).setIssuedAt().setExpirationTime("24h").sign(new TextEncoder().encode(secret));
  },
...
// src/x-api.ts
var X_API = "https://api.twitter.com";
var xApi = {
  async getUserInfo(accessToken) {
    ...
    if (!res.ok)
      throw new Error(`getUserInfo ${res.status} ${await res.text()}`);
  ...
  async postTweet(accessToken, tweet) {
    ...
    if (!res.ok)
      throw new Error(`postTweet ${res.status} ${await res.text()}`);
    return res.json();
  }
};
...
app.get("/auth/callback", async (c) => {
  ...
  try {
    ...
    const user = {
      ...
      tokenExpiry: new Date(Date.now() + tokens.expires_in * 1e3).toISOString(),
    ...
    try {
      ...
      if (userStr) {
        ...
        const tweetResponse = await xApi.postTweet(user.accessToken, {
          text: post.content,
          media: post.mediaUrls.length > 0 ? { media_ids: post.mediaUrls } : void 0
...
async function processScheduledPosts(env) {
  ...
  try {
    ...
    for (const key of keys) {
      ...
      log(`Server Time: ${(/* @__PURE__ */ new Date()).toISOString()}`);
      const userData = await env.KV_TOKENS.get(key.name);
      if (!userData)
        continue;
      const user = JSON.parse(userData);
      const tokenExpiry = new Date(user.tokenExpiry);
      const needsRefresh = tokenExpiry.getTime() - Date.now() < 3e5;
      ...
      const postsToSend = posts.filter((post) => {
        ...
        const isTime = userWallTime >= scheduledDate && userWallTime.getTime() - scheduledDate.getTime() < 24 * 60 * 60 * 1e3;
        log(`Post ${post.id}: Scheduled ${post.scheduledFor} (Wall), User Now ${userWallTime.toISOString()} (Wall in ${post.timezone}). Ready? ${isTime}`);
        if (!isTime) {
          log(`Skipping post ${post.id}: Diff ${(userWallTime.getTime() - scheduledDate.getTime()) / 1e3}s`);
        }
        return isTime;
      });
      ...
      for (const post of postsToSend) {
        try {
          let inReplyToTweetId;
          if (post.parentId) {
            const parentPost = posts.find((p) => p.id === post.parentId);
            inReplyToTweetId = parentPost?.postedTweetId || void 0;
          }
          const tweetResponse = await xApi.postTweet(accessToken, {
            text: post.content,
            media: post.mediaUrls.length > 0 ? { media_ids: post.mediaUrls } : void 0,
            reply: inReplyToTweetId ? { in_reply_to_tweet_id: inReplyToTweetId } : void 0
          });
          const updatedPost = {
            ...post,
            status: "sent",
          ...
          posts[postIndex] = {
            ...posts[postIndex],
            status: "draft",
            // Reset to draft so user can reschedule
            updatedAt: (/* @__PURE__ */ new Date()).toISOString()
          };
        }
      }
...
__name(processScheduledPosts, "processScheduledPosts");
var src_default = {
  fetch: app.fetch,
...
// .wrangler/tmp/bundle-UCjCsK/middleware-insertion-facade.js
...
// .wrangler/tmp/bundle-UCjCsK/middleware-loader.entry.ts
